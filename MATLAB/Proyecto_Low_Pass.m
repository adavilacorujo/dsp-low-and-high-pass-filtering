...
for i=1:length(fltrs)
    figure
    filter = cell2mat(fltrs(i));
    freqz2(filter);
    [row, col] = size(filter);
...
%% Filter each image
filtered_images = apply_filters(fltrs, images);

save_images(fltrs, images, filtered_images,names);
display_images(fltrs, images, filtered_images);
...
function no_use = save_images(filters, images, filtered_images, names)
    ...
    for i=1:sze             
        [row, col] = size(cell2mat(filters(j)));   
        title(['Filter is', num2str(row), 'x',num2str(col)]);
        imwrite(cell2mat(filtered_images(i)), strcat('..\IMAGES\low_pass\', num2str(row), 'x',num2str(col), cell2mat(names(x))));
        if mod(i, 5) == 0 && i ~= 1 
            j = 1;
            x = x + 1;
        else
